...
const ReflectionForm = () => {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      console.log("The value of repData is ", repData);
      console.log("the value of success is ", repData.success);
    ...
    <>
      <form onSubmit={form.handleSubmit(onSubmit)} className="m-3">
            ...
            <Select onValueChange={(value) => form.setValue("name", value)}>
              ...
              <SelectContent className="bg-white">
                {usersAll.map((user, index) => {
                  return (
                    <SelectItem key={Number(index)} value={user}>
                      {user}
                    </SelectItem>
                  );
                })}
        ...
        {/* buttons */}
          ...
          {
            successFlag && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">

                <div className="flex flex-col bg-green-300 border-[1.5px] border-green-400 p-6 shadow-xl rounded-lg  ">
                    <h2 className="mb-4">Success ! The form is filled </h2>
                    <button className="bg-black p-1 text-white " onClick={()=>{setSuccessFlag(false)}}>Close</button>
                </div>
        ...
        {errorMessage === null ? "" :  (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                ...
                <div className=" bg-white p-6 rounded-lg shadow-xl text-center">
                    ...
                    <h2 className="font-bold text-red-900 text-md">Error</h2>

                    <h2 className="text-red-500 mb-4">{errorMessage}</h2>
                    <Button className="bg-primary text-white rounded-lg py-2 px-4" onClick={()=>{setErrorMessage(null)}}> Close </Button>
