...
const ReflectionForm = () => {
  const [secondaryHabitDetails, setSecondaryHabitDetails] =
    ...
    try {
      ...
      if (repData.success) {
        ...
        toast.error("Submission failed", {
          description:
            repData.message || "An error occurred while submitting the form",
        });
      }
    } catch (error) {
      console.log(
  ...
  const fetchSecondaryHabitDetails = async (
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/user/get-user-details/${user?.id}`
      );

      const repData = await response.json();

      const userDetails = repData.data;

      if (userDetails) {
        const secondaryHabitId = userDetails?.secondaryHabit[0];

        if (secondaryHabitId) {
          try {
            const response = await fetch(
              `${process.env.NEXT_PUBLIC_BACKEND_URL}/user/custom-value/${secondaryHabitId}`
            );

            const repData = await response.json();

            console.log("this is the value of secondaryHabitId", repData);

            setState(repData.data.secondaryHabit);
          } catch (error) {
            console.error("Error fetching secondary habit details:", error);
          }
        }
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (user?.id) {
      fetchSecondaryHabitDetails(setSecondaryHabitDetails);
    }
  }, [user?.id]);
    ...
    <>
      <form onSubmit={form.handleSubmit(onSubmit)} className="m-3">
        ...
        {secondaryHabitDetails && (
          <div className="bg-gray min-h-36 p-6 rounded-3xl mt-5">
            <h3 className="text-xl text-text-100 mb-4">
              Enter your {secondaryHabitDetails.habitName} value
            </h3>
            <input
              type="number"
              placeholder="Enter the value"
              className="w-full bg-white rounded-lg py-3 px-4 text-gray-600 shadow-xl h-12"
              {...form.register("customNumber", {
                valueAsNumber: true,
                required: "Please enter a value",
              })}
            />
            {form.formState.errors.customNumber && (
              <p className="text-red-500 text-sm mt-1">
                {form.formState.errors.customNumber.message}
              </p>
            )}
          </div>
        )}
